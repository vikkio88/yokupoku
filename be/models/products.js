...
const {
  ...
  PRODUCT_TYPES,
  SLUGEABLE_STORES,
} = require("yokupoku-shared/enums/db");
...
const products = {
  ...
  get({ range = [0, 9], sort = ["id", "asc"], filter = null }) {
    const [lower, upper] = range;
    const limit = upper - lower;
    const offset = lower;
    const query = db(prodTable).select("id", "name", "type", "meta", "slug");
  ...
  async getWithReviews() {
    ...
    const result = await db(TABLES.PRODUCTS)
      ...
      .then((rows) =>
        rows.map((row) =>
          genericProductsFormat.feDataSelect(row, indexedReviews)
        )
      );
...
const nonGamesProducts = {
  ...
  async delete(id) {
    const result = await db(prodTable).where("id", id).delete();
    return result === 1;
  },
};
